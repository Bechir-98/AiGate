...
async def save_chat_history(redis_client, session_id: str, history: list):
    history_key = f"session:{session_id}:history"
    trimmed_history = history[-10:]  # Garde les 10 derniers messages
    await redis_client.setex(history_key, 1800, json.dumps(trimmed_history))

async def process_gateway_chat(request, input_data: Input, background_tasks, db) -> GatewayResponse:
    redis_client = request.app.state.redis
    
    # 1. Gestion de la session
    session_id = input_data.session_id or f"sess_{uuid.uuid4().hex[:12]}"

    # 2. Routage du scanner et anonymisation
    ...
    # 4. Construction du contexte pour LiteLLM
    messages = [
        {
            "role": "system",
            "content": (
    ...
    # 6. Sauvegarde de l'historique anonymisé
    ...
    # 7. Désanonymisation de la réponse finale
